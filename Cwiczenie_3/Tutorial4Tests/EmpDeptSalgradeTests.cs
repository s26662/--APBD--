...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(c => c.DeptNo == 30).OrderByDescending(n => n.Sal).ToList(); 
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(c => depts
                                                .Where(d => d.Loc == "CHICAGO")
                                                .Select(d => d.DeptNo)
                                                .Contains(c.DeptNo) ).ToList(); 
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select(empGroup => new
        {
            DeptNo = empGroup.Key,
            AvgSal = empGroup.Average(e => e.Sal),
        }).ToList(); 
